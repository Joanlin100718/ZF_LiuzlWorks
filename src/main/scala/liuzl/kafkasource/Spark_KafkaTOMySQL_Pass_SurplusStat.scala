...
*
* 该程序只存储“系统运维”数据中除了Stat的topic内的数据
*
*
*
* */
...
object Spark_KafkaTOMySQL_Pass_SurplusStat {
...
//    val offsetList = List(("AIOPS_ETE_SERVSQLTOPO", 0, 1L),("AIOPS_ETE_SERVSTRTOPO", 2, 25L),("AIOPS_ETE_SERVSQLTOPO", 0, 28L),("AIOPS_ETE_SERVSQLTOPO", 1, 29L))                          //指定topic，partition_no，offset

    var fromOffsets1: Map[TopicAndPartition, Long] = Map()
    // 定义返回Map值
    val tp = TopicAndPartition("AIOPS_ETE_SERVSQLTOPO",0)

    fromOffsets1 += (tp -> 1L)
    ("AIOPS_ETE_SERVSQLTOPO", 0, 1L)
